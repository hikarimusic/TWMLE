...
- 原因：
    ...
    - 假性高血鉀：hemolyzed sample、leukocytosis、thrombocytosis
- ECG：**peaked T waves** → PR prolongation → P wave 消失 → **QRS widening** → sine wave → VF/asystole
- 緊急處理：
    1. **Calcium gluconate**：穩定心肌膜電位（**最優先，不降 K⁺**）
    2. **Insulin + Glucose**：K⁺ 移入細胞
    3. **NaHCO₃**：K⁺ 移入細胞
    4. **β₂-agonist**：K⁺ 移入細胞
    5. **Kayexalate / Kalimate**：促腸道排 K⁺
    6. **Loop diuretics**：促腎臟排 K⁺
    7. **Hemodialysis**：最有效移除 K⁺ 的方法
...
- 原因：
    - 常見：Primary hyperparathyroidism、Malignancy（PTHrP、osteolytic metastases、1,25-D production by lymphoma）
    - 其他：vitamin D intoxication、granulomatous disease（sarcoidosis）、thiazide diuretics、milk-alkali syndrome、hyperthyroidism、immobilization
- 症狀：Stones（腎結石）, Bones（骨痛/骨折）, Groans（腹痛/便秘/噁心）, Moans（意識混亂/憂鬱）
- ECG：**QT shortening**、Osborn wave
- 治療：
    1. **大量 N/S 輸液**：恢復血容
    2. **Loop diuretics**：促 Ca²⁺ 排泄
    3. **Calcitonin**：快速起效但短暫
    4. **Bisphosphonates**：2–4 天起效，效果持久
    5. **Denosumab**：anti-RANKL，適合 bisphosphonate 禁忌者
    6. **Glucocorticoids**：granulomatous disease、lymphoma
...
- 常見原因（MUDPILES）：
    - **M**ethanol（代謝成 formic acid → 失明）
    - **U**remia
    - **D**KA（或 alcoholic/starvation ketoacidosis）
    - **P**ropylene glycol
    - **I**NH / Iron
    - **L**actic acidosis（tissue hypoxia、liver disease、metformin、malignancy）
    - **E**thylene glycol
    - **S**alicylate（aspirin）
...
- 原因
    ...
    - 維持原因：Cl⁻ depletion（近端 NaHCO₃ 重吸收 ↑）、hypokalemia、mineralocorticoid excess
- 分類依 urine Cl⁻：
    - Chloride-responsive（UCl < 20）：嘔吐、NG suction、利尿劑（停用後）、post-hypercapnia；治療：N/S 輸液（volume repletion + Cl⁻ 補充）
    - Chloride-resistant（UCl > 20）：primary aldosteronism、Cushing's、Bartter/Gitelman、 diuretic use、severe hypokalemia；治療：針對原因
...
| Calcium phosphate | 10–15% | 常與 oxalate 混合 | Radio-opaque | 鹼性 |
| **Struvite（MAP）** | 10–15% | **Staghorn calculus**；**urease-producing bacteria**（Proteus、Klebsiella） | Radio-opaque | **鹼性** |
| **Uric acid** | 5–10% | Rhomboid/rosette crystals | **Radio-lucent** | **酸性** |
| Cystine | 1–2% | Hexagonal crystals；Cystinuria（AR） | Faintly radio-opaque | 酸性 |

- 臨床表現：
    - **腎絞痛（renal colic）**：劇烈腰痛放射至鼠蹊部，陣發性
    - 血尿（gross or microscopic）
    - 噁心嘔吐
    - 若合併感染 → 發燒、膿尿 → **obstructive pyelonephritis
- 診斷：
    - 首選影像：**Non-contrast CT** → 可偵測所有類型結石（包括 uric acid）
    - KUB：可看 radio-opaque stones（不能看 uric acid stones）
    - 超音波：孕婦、小兒首選
- 治療：
    - < 5 mm：自然排出機會高（> 90%）→ 止痛 + hydration + α-blocker
    - 5–10 mm：可嘗試觀察；若無法排出 → 介入
    - \> 10 mm 或 > 2 週無法排出：介入處理
        - **ESWL（體外震波碎石）**：適合 < 2 cm、位於腎盂/上段輸尿管的石頭
        - **Ureteroscopy**：輸尿管結石首選
        - **Percutaneous nephrolithotomy**：> 2 cm 或 staghorn calculus
    - Struvite（staghorn）：必須完全移除 + 抗生素→ PCNL 首選
    - Uric acid stones：尿液鹼化（oral NaHCO₃ 或 potassium citrate）+ allopurinol
- 預防：
    - 所有類型：**增加水分攝取**
    - Calcium oxalate：限草酸食物（菠菜、巧克力）、正常鈣攝取（非限鈣）、potassium citrate、thiazide（減少尿鈣）
    - Uric acid：鹼化尿液 + allopurinol
    - Cystine：鹼化尿液 + tiopronin 或 D-penicillamine（結合 cystine）
