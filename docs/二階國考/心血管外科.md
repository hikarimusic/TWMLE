...
## 一、冠狀動脈疾病（Coronary Artery Disease, CAD）
...
### 1.2 急性冠心症（Acute Coronary Syndrome, ACS）
...
## 二、體外循環（Cardiopulmonary Bypass, CPB）
...
- **David procedure（Valve-sparing root replacement）**：保留自體主動脈瓣，僅置換升主動脈 → 適用於瓣膜本身正常、根部擴大者
...
- **最常見原因**：
    - < 65 歲：bicuspid aortic valve（**最常見先天性心臟異常**）
...
- **聽診**：S2 減弱/消失、**crescendo-decrescendo systolic murmur**（收縮期漸強漸弱雜音），best at right upper sternal border → 傳導至頸部
...
- **治療**：
    - 手術首選：**Aortic Valve Replacement（AVR）**（機械瓣或生物瓣）
...
- **身體檢查**（周邊徵象多，因 wide pulse pressure）：
...
- **治療**：
    - **手術（AVR）**：首選為瓣膜置換；若瓣膜結構許可可行 valve repair
    - 合併主動脈根部擴大 → **Bentall procedure** 或 **David procedure**
    - 急性 AR → **緊急 AVR**；術前可用 Nitroprusside/Inotropes 暫時穩定
    - 慢性 AR 內科治療：vasodilator（Nifedipine、ACEi/ARB）可暫時減輕 regurgitant volume，但**不能取代手術**
...
- **病生理**：LA pressure ↑ → 肺靜脈壓↑ → 肺高壓 → 右心衰竭
    - LA 擴大 → **心房顫動（Afib）**（最常見併發症）→ 血栓 → **中風**
- **聽診**：
    - **Opening snap + low-pitched diastolic rumble**（best at apex, left lateral decubitus position）
    - S1 增強（早期）
    - Opening snap 與 S2 間距越短 → 越嚴重
- **正常瓣膜面積 4–6 cm²；嚴重狹窄 < 1.5 cm²**
- **惡化時機**：Afib 新發、懷孕（血量增加）、感染
- **治療**：
    - 利尿劑、rate control（β-blocker）、抗凝血（Afib → warfarin）
    - **Percutaneous Mitral Balloon Valvuloplasty（PMBV）**：瓣膜仍具可塑性（pliable, non-calcified, no MR, no LA thrombus）
    - 不適合 PMBV → 手術（commissurotomy 或 MVR）

### 3.4 二尖瓣閉鎖不全（Mitral Regurgitation, MR）

- **急性 MR**：
    - MI 後 papillary muscle rupture（**posteromedial papillary muscle** 最常受影響，因單一血管供應，由 PDA 供應）
...
- **治療**：
    - **Mitral valve repair 優於 replacement**（保留 subvalvular apparatus → 維持 LV function）
    - 手術適應症：有症狀的嚴重 MR、無症狀但 LVEF ≤ 60% 或 LVESD ≥ 40 mm、新發 Afib 或肺高壓
...
- **TR（Tricuspid Regurgitation）**：
    ...
    - 聽診：holosystolic murmur at left lower sternal border，**吸氣增強（Carvallo's sign）**
...
|------|------------------------|--------------------------|
| 耐久性 | 終身 | 10–20 年（會退化） |
| 抗凝血 | **終身 Warfarin**（INR 2.5–3.5 主動脈位；3.0–3.5 二尖瓣位） | 術後 3–6 個月，之後可能不需要 |
| 適用族群 | 年輕人（< 50 歲）、已需長期抗凝者 | 老年人（> 65 歲）、不適合/不願抗凝、懷孕計畫者 |
| 常見類型 | St. Jude（bileaflet，最常用） | 豬瓣膜、牛心包膜 |

- **懷孕**：Warfarin 有致畸胎性（warfarin embryopathy，尤其第一孕期）→ 生物瓣膜較佳或換用 heparin
...
- **手術適應症**：
    ...
    - Prevention of embolism（大 vegetation > 10 mm + embolic event；vegetation > 15 mm）
    - 人工瓣膜 IE（尤其早期）
...
- **Type A → 緊急手術**（Bentall procedure 或升主動脈置換 ± reimplant coronary）
- **Type B → 藥物控制**：
...
- **Complicated Type B**（臟器灌流不足、破裂、持續疼痛、擴大）→ **TEVAR（Thoracic Endovascular Aortic Repair）** 或手術
...
- **篩檢**：USPSTF 建議 **65–75 歲曾吸菸男性** → 一次腹部超音波
...
## 五、周邊動脈疾病（Peripheral Arterial Disease, PAD）
...
### 5.2 急性肢體缺血（Acute Limb Ischemia, ALI）
...
### 5.3 頸動脈疾病（Carotid Artery Disease）
...
## 六、靜脈疾病（Venous Disease）

### 6.1 下肢靜脈曲張（Varicose Veins）
...
### 6.2 慢性靜脈功能不全（Chronic Venous Insufficiency, CVI）
...
- **治療**：抗凝（LMWH 或 NOAC）為主；療程依 provoked（3 個月）或 unprovoked（≥ 6 個月）決定
- **Phlegmasia alba dolens**：大量 DVT → 全肢白腫、有脈搏
- **Phlegmasia cerulea dolens**：更嚴重 → 全肢發紺（cyanosis）、可能無脈搏 → 可致 venous gangrene → 緊急 catheter-directed thrombolysis 或 thrombectomy
- **IVC filter 適應症**：
    - 有 DVT/PE 且**禁忌抗凝血**
...
### 7.1 胸廓出口症候群（Thoracic Outlet Syndrome, TOS）
...
### 7.2 肺栓塞（Pulmonary Embolism, PE）—外科觀點
...
## 八、先天性心臟病（Congenital Heart Disease）
...
## 九、心包膜疾病（Pericardial Disease）

### 9.1 心包填塞（Cardiac Tamponade）
...
### 9.2 縮窄性心包炎（Constrictive Pericarditis）
...
### 10.1 Intra-aortic Balloon Pump（IABP）
