...
- 大多數不增加乳癌風險
- **Atypical ductal hyperplasia（ADH）及 atypical lobular hyperplasia（ALH）→ 乳癌風險增加 4–5 倍**
    - Core biopsy 發現 ADH/ALH → **需進一步做 excisional biopsy**（因 core biopsy 可能 underestimate，旁邊可能有 DCIS/invasive cancer）
...
- 治療：**wide local excision with ≥ 1 cm margin**（不需腋窩淋巴結廓清，因為很少淋巴轉移）
    - Malignant phyllodes 轉移以**血行轉移**為主（→ 肺）
    - 若反覆復發或 malignant 且無法取得足夠 margin → mastectomy
- **局部復發率高** → margin 很重要

### 2.6 乳房膿瘍（Breast Abscess）

- **哺乳期（lactational）**：最常見菌 = **S. aureus** → 先試 antibiotics → 若形成膿瘍 → I&D 或 aspiration
    - 不一定需要停止哺乳（除非 incision 在乳暈附近）
- **非哺乳期（non-lactational/periareolar）**：常與 smoking 有關、常見菌包括 anaerobes → 抗生素需涵蓋 anaerobes
...
- 基因：**BRCA1**（染色體 17）、**BRCA2**（染色體 13）→ 終生乳癌風險 50–80%
    - BRCA1 → 乳癌 + 卵巢癌；**BRCA1 相關乳癌多為 TNBC（basal-like）**
...
- Chemoprevention：**Tamoxifen**（premenopausal 或 postmenopausal）或 **Aromatase inhibitor**（postmenopausal only）→ 高風險者（如 BRCA 帶原者、ADH/ALH）
- 預防性雙側乳房切除術（risk-reducing bilateral mastectomy）：BRCA 帶原者可選擇 → 降低風險約 90%
- 預防性雙側卵巢輸卵管切除術（risk-reducing bilateral salpingo-oophorectomy）：BRCA 帶原者建議（35–40 歲完成生育後）→ 降低卵巢癌 + 乳癌風險
...
- **乳癌篩檢的 gold standard / 最重要的篩檢工具**
- 一般風險女性：各指引略有不同
    - 美國：40–45 歲開始，每 1–2 年
    - 台灣國健署：**45–69 歲**每 2 年一次（二等親內有乳癌 → 40 歲開始）
...
- **年輕女性用超音波、年長者用攝影的原因**：年輕乳腺緻密（dense breast）→ mammography 敏感度低（白色乳腺中找白色腫瘤）；年長者乳腺被脂肪取代 → mammography 效果好
- 孕婦亦以超音波為首選（無輻射）
...
| Vacuum-assisted biopsy（如 Mammotome） | 大口徑、取組織量多，適合微小鈣化切片 |
| Excisional biopsy | 最準確但為手術方式，僅在 core biopsy 不確定時使用 |

### 5.2 Triple Test（三合一評估）

- 結合：(1) 臨床觸診 (2) 影像（mammography/ultrasound）(3) 切片
- 三者一致良性 → 惡性機率極低 → 追蹤
- 任一不一致 → 需進一步評估/手術切片
...
- 治療：
    ...
    - 但做 mastectomy 時可考慮同時做 **sentinel lymph node biopsy（SLNB）**（因為 mastectomy 後無法再做 SLNB）
...
| Inflammatory breast cancer | 1–5% | **最惡性**，見下方 |

### 6.3 炎性乳癌（Inflammatory Breast Cancer）

- **臨床診斷**（非病理分類）
- 表現：乳房瀰漫性紅腫、warmth、**peau d'orange（橘皮樣變化）**、無明顯腫塊
- 病理：dermal lymphatic invasion by tumor cells
- **TNM staging：至少 T4d → Stage IIIB 以上**
...
| T2 | > 2 cm, ≤ 5 cm |
| T3 | > 5 cm |
| T4 | 侵犯胸壁(T4a)、皮膚(T4b)、兩者(T4c)、inflammatory(T4d) |
...
- **Stage III**：T3N1, any T4, any N2-N3
- **Stage IV**：any M1（遠處轉移）
- 乳癌最常見遠處轉移部位：**骨骼** > 肺 > 肝 > 腦
...
| **Triple negative（TNBC）** | − | − | 高 | **最差** | 化療為主（無荷爾蒙/標靶靶點），BRCA 突變者可用 PARP inhibitor |
...
### 9.2 前哨淋巴結切片（Sentinel Lymph Node Biopsy, SLNB）
...
- SLN 陽性（≤ 2 顆 macro-metastasis + BCT + 計畫全乳放療）→ 根據 **ACOSOG Z0011 trial** → 可省略 ALND
- SLNB 禁忌：clinically node-positive（cN+, 需先做 ALND 或 neoadjuvant 後 re-evaluate）、inflammatory breast cancer

### 9.3 腋窩淋巴結廓清（ALND）併發症
...
### 9.4 乳房重建（Breast Reconstruction）
...
- 方法：
    - **Implant-based（植入物）**：tissue expander → 永久 implant（silicone/saline）→ 手術較簡單、恢復快
    ...
    - 自體組織：更自然、耐放療、持久；但手術複雜、恢復久
- **放療對重建的影響**：增加 capsular contracture（植入物）、flap fibrosis → 需放療者多建議 delayed reconstruction 或 autologous flap
- **Nipple-sparing mastectomy（NSM）**：保留乳頭乳暈皮瓣 → 適合腫瘤離乳頭距離足夠、無乳頭侵犯者 → cosmetic 效果最佳
...
### 10.1 化學治療（Chemotherapy）

- 常用方案：AC（Adriamycin + Cyclophosphamide）→ T（Taxane）
- **Anthracycline（Doxorubicin/Adriamycin）副作用：cardiotoxicity**（cumulative dose-related → dilated cardiomyopathy）
- 化療適應症：TNBC、HER2+、high-risk Luminal B、lymph node positive

### 10.2 荷爾蒙治療（Endocrine Therapy）
...
- **Trastuzumab（Herceptin）**：HER2+ 乳癌 → 1 年療程
...
### 10.5 Neoadjuvant Therapy（術前治療）

- 目的：腫瘤降期（downstaging）→ 使原本不適合 BCT 者可以做 BCT
- 適用：locally advanced breast cancer、inflammatory breast cancer、TNBC/HER2+（反應率高）
- Pathologic complete response（pCR）：與預後正相關
...
- 臨床檢查：前 5 年每 3–6 個月，之後每年
- **同側（BCT 後）或對側乳房 mammography**：每年
- **不建議常規做全身影像檢查（CT、bone scan、PET）**：僅在有症狀時安排
