...
- 分期：CT chest/abdomen/pelvis；直腸癌加 **MRI pelvis**（評估 mesorectal involvement）+ **ERUS（endorectal ultrasound）**
...
| I | T1-T2, N0 | 侵犯黏膜下/肌肉層 |
| II | T3-T4, N0 | 穿過肌肉層，無 LN |
| III | any T, N+ | 有淋巴結轉移 |
| IV | M1 | 遠端轉移 |
...
- 依腫瘤距肛門口距離：
    - 上段直腸（> 10–12 cm from anal verge）→ **Anterior resection** with TME
...
- 直腸癌遠端切除邊界：≥ **1–2 cm**（不需像結腸 5 cm）

#### Neoadjuvant 治療（直腸癌）

- **局部進展直腸癌（T3/T4 或 N+）**→ **術前 neoadjuvant CCRT**（concurrent chemoradiotherapy）→ 手術 → adjuvant chemo
- 好處：腫瘤縮小（downstaging）、提高括約肌保留率、降低局部復發

### 2.10 輔助化療

- **Stage II**：routine 不需要，但高風險者可考慮（T4、poorly differentiated、LVI、perineural invasion、取 < 12 LN、obstruction/perforation）
- **Stage III（N+）**：術後 adjuvant chemotherapy（**FOLFOX = 5-FU + leucovorin + oxaliplatin**）
- **Stage IV with resectable liver/lung metastasis**：可手術切除 + 化療 → 有機會治癒

### 2.11 術後追蹤

- **CEA**：每 3–6 個月（前 2–3 年）
- 大腸鏡：術後 1 年 → 3 年 → 之後每 5 年
- CT：每年（前 3–5 年）
- 直腸癌加做 pelvic CT/MRI
...
- 肝臟是大腸癌最常見的遠端轉移部位
- 若可切除（resectable）→ **肝臟切除（hepatic metastasectomy）** → 5 年存活率 ~40%
...
- 典型順序：**periumbilical pain（visceral，模糊）→ 轉移至 RLQ（McBurney's point：臍與右前上髂棘連線外 1/3）**
...
- **憩室（Diverticulum）**：大腸壁向外突出
    - 西方型：**假性憩室（pseudodiverticulum）**→ 只含黏膜及黏膜下層突出（不含肌肉層）
...
- **複雜性（complicated）**：abscess > 3-4 cm → CT-guided percutaneous drainage + 抗生素
- **手術**：
...
| II | 排便時脫出，可**自行**回縮 | 橡皮圈結紮（rubber band ligation, RBL）、硬化劑注射 |
...
- 急性 vs 慢性（> 6–8 週）：慢性 → sentinel pile（外）+ hypertrophied papilla（內）+ exposed IAS fibers
- 病生理：**內括約肌痙攣** → 血流↓ → 不癒合 → 惡性循環
- 治療：
    - **一線：保守**（高纖、坐浴、軟便劑）+ **Topical nitroglycerin（GTN 0.2%）**或 **topical calcium channel blocker（diltiazem）**→ 放鬆 IAS
    - **Botox injection**：暫時性 IAS 麻痺
    - **手術：Lateral internal sphincterotomy（LIS）**→ 治癒率最高（>95%），但有失禁風險

### 7.3 肛門膿瘍（Perianal/Anorectal Abscess）

- 來源：**肛腺（anal gland）感染** → 起源於齒狀線的 intersphincteric space → **Cryptoglandular theory**
- 分類（由常見到少見）：**Perianal**（最常見）> Ischiorectal > Intersphincteric > Supralevator
- 治療：**切開引流（I&D）**→ 不等、不用抗生素就好（除非蜂窩性組織炎、免疫低下、DM、瓣膜置換）
- 約 30-50% 膿瘍 → 日後形成 **肛門瘻管（fistula-in-ano）**

### 7.4 肛門瘻管（Fistula-in-ano）
...
- 治療：
    - Simple / low（intersphincteric, low trans-sphincteric）→ **Fistulotomy**（切開瘻管）
    - Complex / high → **Seton placement**（引流線）、LIFT procedure、advancement flap → 避免損傷括約肌致失禁
...
- 治療：**Nigro protocol = concurrent chemoradiotherapy（5-FU + Mitomycin C + radiation）**
    - **不做手術！（初始治療非手術）** → 治癒率 80-90%
    - APR 僅用於 Nigro 失敗後的 salvage therapy
...
- 好發：老年女性
- 與單純黏膜脫垂區別：全層有同心圓皺褶（concentric rings）；黏膜脫垂為放射狀皺褶（radial folds）
- 治療：
...
- Ileostomy 比 colostomy 更易有 **高排出量 → 脫水、電解質異常（低鈉、低鉀、metabolic acidosis）**

---

## 九、消化道出血（GI Bleeding）

### 9.1 上消化道出血（Upper GI Bleeding）
