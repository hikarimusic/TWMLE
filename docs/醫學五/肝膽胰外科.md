...
### 4.3 TIPS（Transjugular Intrahepatic Portosystemic Shunt）
...
- 大多數膽結石（>80%）為 **無症狀** → 不需治療
- **X-ray**：只有 ~15% 膽結石可在 X-ray 看到（多為含鈣者）
- 超音波：診斷膽囊結石首選工具（敏感度 >95%）

### 6.2 膽絞痛（Biliary Colic）

- 膽結石暫時阻塞膽囊管 → 膽囊壓力↑ → 右上腹/上腹痛
- 特點：進食（尤其油膩食物）後發作、持續 30 分鐘–數小時、可放射至右肩/背部
- 無發燒、無白血球升高
- **Murphy's sign 陰性**（無持續發炎）
- 治療：症狀性膽結石 → **選擇性膽囊切除術（elective cholecystectomy）**

### 6.3 急性膽囊炎（Acute Cholecystitis）

- 90% 因膽結石持續阻塞膽囊管 → 膽囊壁發炎
- 症狀：持續性右上腹痛 >6 hr、發燒、噁心嘔吐
- **Murphy's sign 陽性**（深吸氣時按壓右上腹 → 疼痛加劇 + 吸氣中斷）
- **Sonographic Murphy's sign**：超音波探頭按壓膽囊 → 同上

#### 診斷

- **超音波（首選）**：膽囊壁增厚 >4 mm、膽囊周圍液體、膽結石 + sonographic Murphy's sign
- **HIDA scan（Hepatobiliary Iminodiacetic Acid）**：最準確
    - 膽囊不顯影（non-visualization）→ 膽囊管阻塞 → 診斷急性膽囊炎

#### 治療

- **Laparoscopic cholecystectomy**（腹腔鏡膽囊切除）→ 首選
    - 最好在 **72 小時內** 施行（early cholecystectomy）
- 若病人手術風險太高 → **經皮膽囊引流（percutaneous cholecystostomy）** 暫時緩解

#### 併發症

- **膽囊壞疽（Gangrenous cholecystitis）**：膽囊壁缺血壞死 → 穿孔風險高 → 緊急手術
...
### 6.4 無結石膽囊炎（Acalculous Cholecystitis）

- 佔急性膽囊炎約 5–10%
- 好發：ICU 重症病人、TPN、燒傷、外傷、長期禁食
- 機制：膽汁淤滯 + 膽囊缺血
- 診斷：US/CT 膽囊壁增厚 + 膽囊周圍液體，無結石；HIDA 不顯影
- 治療：**經皮膽囊引流** 或手術切除
- 壞疽/穿孔率比結石性膽囊炎更高
...
- 預防：**Critical View of Safety（最重要！）**
...
- X-ray 經典三徵（**Rigler's triad**）：
    1. 小腸阻塞（SBO）
    2. 膽道內氣體（pneumobilia / aerobilia）
    3. 異位結石（ectopic gallstone）
- 治療：手術取出結石（enterolithotomy）± 廔管修補 ± 膽囊切除

### 6.7 Mirizzi Syndrome

- 膽囊管或膽囊頸的結石壓迫 **CHD（總肝管）** → 阻塞性黃疸
- 分型（Csendes classification）：
    - Type I：外部壓迫
    - Type II–IV：膽囊膽道廔管（不同程度 CHD 受損）
- 治療：膽囊切除 ± 膽道修補（嚴重者需 Roux-en-Y hepaticojejunostomy）

### 6.8 瓷化膽囊（Porcelain Gallbladder）

- 膽囊壁鈣化（mural calcification）
- 與 **膽囊癌** 有關聯 → 建議 **預防性膽囊切除**
- X-ray 可見膽囊壁鈣化
...
## 九、膽道囊腫（Choledochal Cyst）

### 9.1 分型（Todani Classification）
...
## 十一、急性胰臟炎（Acute Pancreatitis）

### 11.1 病因

- **最常見**：**膽結石（Gallstone，~40%）** 和 **酒精（Alcohol，~30%）** 合佔 70–80%
- 其他：高三酸甘油脂（TG > 1000 mg/dL）、ERCP 後、藥物（Azathioprine、Valproic acid、Didanosine、Thiazide）、外傷、高血鈣、感染（Mumps）、autoimmune、Pancreas divisum、idiopathic
- 記憶口訣：**I GET SMASHED**
    - Idiopathic, Gallstones, Ethanol, Trauma, Steroids, Mumps, Autoimmune, Scorpion sting, Hyperlipidemia/Hypercalcemia/Hypothermia, ERCP, Drugs

### 11.2 診斷標準

需符合以下 **三項中的兩項**：
1. 典型腹痛（上腹痛放射至背部、前彎緩解）
2. **Lipase 或 Amylase ≥ 3 倍正常上限**（Lipase 較 specific）
3. 影像學發現

- 理學檢查特殊徵象（提示重症/出血性壞死性胰臟炎）：
...
| **BISAP** | 簡便（BUN >25, Impaired mental status, SIRS, Age >60, Pleural effusion） |
| **CT Severity Index（Balthazar）** | CT 分級 + 壞死程度 |
...
- **核心治療**：積極輸液復甦（Lactated Ringer's）+ 止痛 + NPO → early enteral feeding（可耐受即開始）
...
- **胰臟假性囊腫（Pancreatic Pseudocyst）**
    ...
    - 內含 **amylase-rich fluid**
    - 無壞死組織
    - 治療：<6 cm 且無症狀 → 觀察；有症狀/增大/併發 → 引流
...
#### 胰臟導管腺癌（Pancreatic Ductal Adenocarcinoma, PDAC）

- 胰臟最常見惡性腫瘤（>85%）；預後極差（5 年存活率 ~10%）
- **好發位置：胰頭（60–70%）** > 體部 > 尾部
- 危險因子：吸菸（最重要可改變因子）、慢性胰臟炎、DM（new-onset DM 可為早期表現）、家族史（BRCA2、PALB2、CDKN2A、Lynch syndrome）、肥胖

##### 臨床表現

- **胰頭癌**：**無痛性阻塞性黃疸**（最典型）、體重下降、新發 DM、背痛（retroperitoneal invasion）
    - **Courvoisier's sign**：可觸及的無痛性膽囊 + 黃疸
    - Trousseau syndrome（migratory thrombophlebitis）：遊走性血栓靜脈炎 → 提示隱匿性癌症
- **胰體/尾癌**：症狀較晚出現 → 診斷時多已晚期；常以背痛、體重減輕表現

##### 診斷

- **CT（pancreatic protocol, thin-cut）**：首選影像；評估腫瘤大小、血管侵犯、轉移
- **EUS + FNA**：用於組織診斷或 CT 看不清的小腫瘤
- **CA 19-9**：最常用腫瘤標記（Lewis antigen 陰性者不會產生 → 約 5–10% 假陰性）
    - 用於追蹤治療反應，非篩檢工具
- ERCP：**double duct sign**（CBD + 主胰管同時擴張）→ 提示壺腹周圍腫瘤

##### 可切除性評估（Resectability）

| 分類 | 動脈 | 靜脈（SMV/PV） |
|------|------|----------------|
| **Resectable** | 無接觸 | 無接觸或 ≤180° 且管腔規則 |
| **Borderline resectable** | 接觸 SMA/celiac ≤180°；接觸 CHA 但不到 proper hepatic a. | >180° 或管腔不規則/血栓 |
| **Locally advanced（不可切除）** | >180° SMA 或 celiac axis | 無法重建之 SMV/PV |
| **Metastatic** | — | — |
...
- **可切除**：
    ...
    - **術後輔助化療（adjuvant）**：**modified FOLFIRINOX**（體能佳者）或 **Gemcitabine + Capecitabine**
- **Borderline resectable**：**Neoadjuvant therapy（術前化療 ± 放療）** → 再評估切除
- **Locally advanced / Unresectable**：化療 ± 放療
- **轉移性**：化療（FOLFIRINOX 或 Gemcitabine + nab-Paclitaxel）
- 黃疸緩解：ERCP + biliary stent（術前減黃，若 bilirubin 太高影響化療或手術時機）

##### Whipple Procedure（胰十二指腸切除術）

- 切除：胰頭 + 十二指腸 + 遠端胃（classic）或保留幽門（PPPD）+ CBD + 膽囊 + 區域淋巴結
- 重建（三個吻合）：
    1. **Pancreaticojejunostomy**（或 pancreaticogastrostomy）— 最危險的吻合（胰液漏）
    2. **Hepaticojejunostomy**
    3. **Gastrojejunostomy**（或 duodenojejunostomy in PPPD）
- 手術死亡率：高量中心 <3–5%
- 最嚴重併發症：**Pancreatic fistula（胰液漏）** → ISGPF 分級 A/B/C
    - 診斷：引流液 amylase > 3 倍血清正常上限（POD 3 以後）
    - Grade B/C → 可能需要引流、angioembolization（出血時）、甚至再手術

#### 胰臟囊性腫瘤（Pancreatic Cystic Neoplasms）

| 腫瘤 | 特點 | 惡性風險 | 處理 |
|------|------|----------|------|
| **Serous cystadenoma（SCN / SCA）** | 中年女性；microcystic（蜂巢狀）；**central stellate scar with calcification** | **極低（幾乎不惡變）** | 觀察為主；有症狀 → 手術 |
| **Mucinous cystic neoplasm（MCN）** | **幾乎只有女性**；胰體/尾；不與胰管相通；有 **ovarian-type stroma** | **有惡變潛力** | **全部切除** |
| **IPMN（Intraductal Papillary Mucinous Neoplasm）** | 男女皆有；與胰管相通；**「Fish-eye」papilla 可見黏液流出** | 見下述 | 依亞型和特徵決定 |
| **Solid pseudopapillary neoplasm（SPN / Frantz tumor）** | **年輕女性**；大的 well-demarcated 腫瘤；出血壞死 | 低度惡性 | **手術切除** |
...
- **Branch duct IPMN**：側支型
    ...
    - **Worrisome features**（需進一步 EUS 評估）：
        ...
        - CA 19-9 升高
        - 囊腫增長速率 ≥ 5 mm/2 年
    - 無 high-risk stigmata / worrisome features → 追蹤（依大小決定頻率）

### 13.2 胰臟神經內分泌腫瘤（Pancreatic Neuroendocrine Tumors, pNETs）

| 腫瘤 | 分泌激素 | 典型表現 | 好發位置 | 惡性比例 | 特殊重點 |
|------|----------|----------|----------|----------|----------|
| **Insulinoma** | Insulin | **Whipple's triad**：低血糖症狀 + 血糖 <50 + 進食緩解 | 胰臟任意處 | **<10%（最常為良性）** | 最常見功能性 pNET；確診：72-hr fasting test（高 insulin + 高 C-peptide + 低血糖） |
| **Gastrinoma** | Gastrin | **Zollinger-Ellison syndrome（ZES）**：頑固性/多發性消化性潰瘍、腹瀉 | **Gastrinoma triangle**（CBD、D2-D3 交界、胰頸三點圍成之三角） | **60–90% 惡性** | 25–30% 與 **MEN 1** 相關；診斷：fasting gastrin >1000 pg/mL 或 secretin stimulation test 陽性 |
...
#### MEN（多發性內分泌腺瘤症候群）與胰臟腫瘤

| MEN 類型 | 基因 | 相關腫瘤 | 胰臟相關 |
|----------|------|----------|----------|
| **MEN 1** | **Menin**（11q13），AD | **3P**：Parathyroid（最常見）+ Pituitary + Pancreas | Gastrinoma（最常見功能性）、Insulinoma |
| **MEN 2A** | **RET**，AD | Medullary thyroid ca + Pheochromocytoma + Parathyroid hyperplasia | 無胰臟腫瘤 |
| **MEN 2B** | **RET**，AD | Medullary thyroid ca + Pheochromocytoma + **Mucosal neuromas** + Marfanoid habitus | 無胰臟腫瘤 |

#### Chromogranin A（CgA）

- 所有神經內分泌腫瘤（包括功能性與非功能性）的 **通用腫瘤標記**
- 用於診斷輔助、追蹤治療反應與偵測復發
- 非特異性：PPI 使用、慢性萎縮性胃炎、腎功能不全皆可升高
...
## 十四、胰臟移植（Pancreas Transplantation）
...
### 15.4 ITP（Immune Thrombocytopenic Purpura）
