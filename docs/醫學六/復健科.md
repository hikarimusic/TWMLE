...
| 評估工具 | 評估內容 | 重點 |
|----------|----------|------|
| **FIM（Functional Independence Measure）** | 18 項（13 運動 + 5 認知） | 1–7 分制，住院復健最常用 |
| **Modified Rankin Scale（mRS）** | 整體失能程度 | 中風預後常用，0–6 分 |
| **Brunnstrom Stage** | 中風動作恢復階段 | 1–6 期（見中風章節） |
| **Ashworth / Modified Ashworth Scale** | 痙攣程度 | 0–4 級 |

---

## 二、中風復健（Stroke Rehabilitation）
...
### 2.2 中風後動作恢復——Brunnstrom Stages

| 期別 | 描述 |
|------|------|
| Stage 1 | 弛緩期（Flaccidity）：無自主動作 |
| Stage 2 | 開始出現痙攣與聯合反應（Synergy patterns begin） |
| Stage 3 | **痙攣最強**，只有**聯合動作（synergy）**，無法分離動作 |
| Stage 4 | 痙攣開始減少，出現部分脫離 synergy 的動作 |
| Stage 5 | 痙攣減退，可做出獨立於 synergy 的動作 |
| Stage 6 | 接近正常、協調且速度恢復 |

#### 上肢聯合動作模式（Synergy Patterns）

- **屈肌聯合動作（Flexor synergy）**：肩胛骨後縮上提 + 肩外展外轉 + 肘彎曲 + 前臂旋後 + 腕指屈曲
- **伸肌聯合動作（Extensor synergy）**：肩內收內轉 + 肘伸直 + 前臂旋前 + 腕指屈曲
...
- 定義：velocity-dependent 的肌張力增加（上運動神經元病變）
- **與 rigidity 區別**：rigidity 是 velocity-independent（如巴金森），痙攣有 clasp-knife 現象
- 治療：
    - 物理治療：牽拉、姿位擺放、站立架、副木
    - 藥物：**Baclofen**（GABA-B agonist，口服或鞘內注射）、Tizanidine（α2-agonist）、Dantrolene（作用於肌肉端）、Diazepam
    - **肉毒桿菌素（Botulinum toxin）**：局部注射，抑制 ACh 釋放 → 適用於局部痙攣，效果持續 3–6 個月
    - 鞘內 Baclofen pump（ITB）：嚴重全身痙攣（如脊髓損傷）
    - 手術：選擇性背根切斷術（selective dorsal rhizotomy）、肌腱延長術
...
- 中風後約 30–50% 有吞嚥困難（詳見第十一章）

#### 失語症（Aphasia）

| 類型 | 病灶 | 流暢度 | 理解力 | 複述 |
|------|------|--------|--------|------|
| **Broca（表達型/運動性）** | 左側額葉（Broca's area） | 差（非流暢） | 好 | 差 |
| **Wernicke（接收型/感覺性）** | 左側顳葉（Wernicke's area） | 好（流暢但無意義） | 差 | 差 |
| **Global（整體型）** | 大範圍左半球 | 差 | 差 | 差 |
| **Conduction（傳導型）** | 弓狀束（arcuate fasciculus） | 好 | 好 | **差**（特徵） |
| **Transcortical motor** | Broca 周圍 | 差 | 好 | **好** |
| **Transcortical sensory** | Wernicke 周圍 | 好 | 差 | **好** |

- **Dysarthria（構音障礙）**：語言理解正常但發音/清晰度異常 → 語言治療
...
- **偏癱步態（Hemiplegic gait）**：
    - 患側髖外展外旋、膝過伸（genu recurvatum）、踝蹠屈內翻（equinovarus）
    - **環繞步態（Circumduction gait）**：患側下肢畫弧向前擺動
- 輔具選擇：四腳拐（quad cane）或單拐 → 持於**健側**
...
- **UMN bladder（Reflex bladder）**：損傷在脊髓圓錐（S2–S4）以上 → 痙攣性膀胱、容量小、DSD（逼尿肌-括約肌協調障礙）
    - 治療：**CIC（間歇導尿，黃金標準）** + Oxybutynin（抗膽鹼）
...
| Unstageable | 壞死組織覆蓋，無法判定深度 |

- 預防：**每 2 小時翻身（最重要！）**、減壓床墊、營養充足、輪椅每 15–30 分鐘 pressure relief

#### 其他併發症

- **DVT**：急性期風險極高 → LMWH + IPC + 彈性襪
- **異位性骨化（HO）**：損傷平面以下關節周圍（髖關節最常見）→ Triple-phase bone scan 早期最敏感、ALP↑ → Indomethacin 預防
...
| 症候群 | 損傷位置 | 表現 | 預後 |
|--------|----------|------|------|
| **Central cord** | 中央（頸椎過伸、老年） | 上肢 > 下肢無力，cape-like 感覺喪失 | **最常見不完全SCI**，預後較好 |
| **Anterior cord** | 前脊髓動脈 | 運動+痛溫覺喪失、**本體感覺保留** | **預後最差** |
| **Brown-Séquard** | 半側脊髓 | 同側運動+本體覺喪失；對側痛溫覺喪失 | **預後最好** |
| **Conus medullaris** | 脊髓圓錐（L1–L2） | LMN 膀胱/腸道障礙、鞍部感覺喪失 | — |
| **Cauda equina** | 馬尾神經 | LMN 下肢無力（不對稱）、鞍部麻木、膀胱障礙 | **外科急症** |

### 3.6 脊髓休克（Spinal Shock）

- 損傷後暫時性 areflexia + 弛緩性癱瘓
- 結束標誌：**球海綿體反射（bulbocavernosus reflex）恢復**（通常 24–72 小時）
- 脊髓休克結束後 → 出現 UMN 徵象（痙攣、反射亢進）

### 3.7 外傷性腦損傷（Traumatic Brain Injury, TBI）
...
- 共 8 級，描述 TBI 後認知行為恢復過程
- **Level IV（Confused-agitated，躁動混亂期）**為復健管理重點：安全為首要，減少刺激，使用 Amantadine 促清醒
- Level I–III：無反應 → 局部反應；Level V–VI：混亂但漸改善；Level VII–VIII：自動/適當反應
...
- 治療：RICE → 早期功能性復健（ROM + 本體感覺訓練）；完全斷裂 rarely 需手術
...
- 大多數為非特異性，自限性
- **紅旗徵象**：發燒/體重減輕（感染/腫瘤）、夜痛、大小便功能障礙（cauda equina syndrome）、進行性神經學缺損
...
### 4.3 截肢與義肢（Amputation & Prosthetics）

#### 截肢平面

- 保留越長越好（能量消耗越少）
- **BKA（Below-Knee / Transtibial）**最常見
- BKA 理想殘肢長度：脛骨粗隆以下 12–15 cm（約脛骨全長的 **40%**）
...
- **Socket（承筒）**：與殘肢接觸，**義肢最重要的部分**
- 膝關節（AKA 用）、踝/足部件

### 4.4 骨質疏鬆（Osteoporosis）

- T-score ≤ -2.5（DEXA）→ 骨質疏鬆；-1.0 ~ -2.5 → 骨質減少
- 好發骨折：脊椎壓迫性骨折、髖骨骨折、遠端橈骨（Colles' fracture）
- 復健：負重運動（weight-bearing exercise）、平衡訓練（預防跌倒）、背伸肌強化

---

## 五、周邊神經損傷與電學診斷

### 5.1 周邊神經損傷分類——Seddon & Sunderland

| Seddon | Sunderland | 損傷 | Wallerian 退化 | 恢復 |
|--------|------------|------|----------------|------|
| **Neurapraxia** | Grade I | 局部脫髓鞘，軸突完整 | 無 | 完全恢復（數天–數週） |
| **Axonotmesis** | Grade II–IV | 軸突斷裂 ± 內膜/外膜損傷 | 有 | 可恢復（約 **1 mm/day = 1 inch/month**） |
...
| **Median nerve（C6-T1）** | 肱骨髁上骨折、CTS | **Hand of benediction**（握拳時 2-3 指無法屈曲）、thenar 萎縮 |
| **Ulnar nerve（C8-T1）** | 肘部撞擊/骨折 | **Claw hand**（4-5 指 MCP 過伸 + IP 屈曲）、Froment's sign、interossei 無力 |
| **Long thoracic nerve（C5-7）** | 手術/外傷 | **Winging of scapula**（翼狀肩胛） |
...
| 類型 | 神經根 | 機制 | 表現 |
|------|--------|------|------|
| **Erb-Duchenne palsy** | C5–C6 | 肩膀被拉開（最常見） | 「**侍者手（waiter's tip）**」：肩內收內轉+肘伸直+前臂旋前 |
| **Klumpke palsy** | C8–T1 | 手臂被上拉 | 手部肌萎縮、爪形手 ± **Horner's syndrome**（T1 交感） |
...
#### M-wave、F-wave、H-reflex 傳遞路徑

- **M-wave（CMAP）**：刺激點 →（**順向**沿運動神經）→ NMJ → 肌肉收縮 = 直接運動反應
- **F-wave**：刺激點 →（**逆向**沿運動神經）→ 前角細胞（被逆向激活）→（**再順向**）→ 肌肉 = 晚期小波。全程只走**運動神經**，不經突觸反射弧 → 評估近端運動傳導（神經根/近端段）
- **H-reflex**：刺激 → 沿**感覺神經（Ia afferent）**→ 脊髓後角 → **突觸** → 前角運動神經元 → 順向 → 肌肉 = 電生理的單突觸反射弧（等同 Achilles tendon reflex）。最常用於評估 **S1 神經根病變**（刺激脛神經、記錄 soleus）
...
- **建議檢查時機：損傷後 3 週**（讓 Wallerian degeneration 完成以見 fibrillation/PSW）
...
- 復健：維持功能為目標、適度運動（不超量）、輔具、吞嚥困難 → **PEG（percutaneous endoscopic gastrostomy）**、呼吸衰竭 → NIV
- 避免 overwork weakness

### 6.5 肌失養症（Muscular Dystrophy）

#### Duchenne Muscular Dystrophy（DMD）

- **X-linked recessive**，dystrophin **完全缺乏**
- 男孩 2–5 歲發病，近端肌無力
- **Gower's sign**：從地上爬起需用手撐膝「爬身體」
- **假性肥大（pseudohypertrophy）**：腓腸肌（脂肪/纖維取代）
- CK 極高、10–12 歲失去行走能力
- 心肌病變+呼吸衰竭為主要死因
- 治療：Prednisolone 延緩進程、復健維持功能、呼吸照護

#### Becker Muscular Dystrophy（BMD）

- X-linked，dystrophin **部分保留** → 症狀較輕、進程較慢，通常 >16 歲仍可行走
...
- **Spastic hemiplegia**：一側肢體（足月兒、腦梗塞），上肢通常 > 下肢 → **可以行走**
...
- PT：NDT（Bobath）、肌力訓練
- OT：ADL 訓練、手部功能、輔具
- 痙攣管理：肉毒桿菌注射、Baclofen、SDR
- AFO 矯正馬蹄足
- **GMFCS（Gross Motor Function Classification System）**：Level I–V 評估粗動作功能

### 7.4 發展遲緩與早期療育

- **Denver II**：粗動作、細動作-適應、語言、個人-社會四大領域
...
- 內容：
    ...
    - 氣道清潔：**姿位引流（postural drainage）**→ 原則：**病變肺葉放在上方**（讓分泌物靠重力往下流向大氣管排出）；拍痰（percussion）、有效咳嗽
    - 能量保存策略
...
- **Gate Control Theory**：大纖維（Aβ，觸覺）可在脊髓後角抑制小纖維（Aδ/C，痛覺）→ TENS 原理

### 9.2 複雜性區域疼痛症候群（CRPS）

- **Type I（RSD）**：無明確神經損傷；**Type II（Causalgia）**：有明確周邊神經損傷
...
- **淺層熱**（深度 1–2 cm）：熱敷包、石蠟浴、紅外線
- **深層熱**：
    - **超音波（Ultrasound）**：深達 5 cm；1 MHz（深層）、3 MHz（淺層）
        - 禁忌：急性發炎、腫瘤、生殖腺、孕婦腹部、生長板、缺血區、感覺缺損區
    - 短波透熱、微波透熱
...
| **HKAFO** | 高位脊損（T10 以上） | 穩定髖膝踝，需拐杖 |

- Posterior leaf spring AFO：適用於單純垂足，輕薄
- Solid AFO：固定踝關節，適用於痙攣性蹠屈+內翻
...
- **Soft collar**：提醒為主；**Philadelphia collar**：限制頸椎屈伸
- **Halo vest**：**最強固定**（不穩定頸椎骨折如 C2 齒突骨折）
- **TLSO**：胸腰椎壓迫性骨折；**LSO**：下背痛/腰椎術後

### 10.4 輪椅處方

- 座寬：臀寬 + 2.5 cm；座深：大腿後方至膝窩 − 5 cm
- **Tilt-in-space**：全身傾斜（角度不變）→ 壓力重新分配
- **Recline**：背靠後仰 → 休息/導尿
- C4：sip-and-puff / head control 電動輪椅；C5：joystick 電動輪椅

### 10.5 步行輔具

- 穩定性排序：**平行桿 > 助行器 > 腋下拐 > 前臂拐（Lofstrand）> 四腳拐 > 單拐**
- 拐杖持於**健側**、與**患側腳同時前進**
...
- **VFSS（Videofluoroscopic Swallowing Study，電視透視吞嚥攝影）**：**gold standard**，可動態觀察食團通過各分期，確認有無吸入
...
| 類型 | 定義 | 特點 |
|------|------|------|
| **等長收縮（Isometric）** | 肌肉出力但**關節角度不變**（長度不變） | **骨折/術後固定期最適合**（不移動關節即可維持肌力）；注意：會升高血壓（Valsalva），心臟病患慎用 |
| **等張收縮（Isotonic）** | 肌肉出力且**關節有動作**（長度改變） | 含向心（concentric，肌肉縮短）與離心（eccentric，肌肉拉長）；**離心收縮產生最大力量、最易造成 DOMS** |
| **等速收縮（Isokinetic）** | 關節以**固定角速度**移動（需專用儀器） | 全 ROM 皆為最大阻力、用於復健後期與運動訓練 |

### 12.2 肌肉適能訓練原則

- **肌力（Strength）**：高阻力、低重複次數（如 6–8 RM）
- **肌耐力（Endurance）**：低阻力、高重複次數（如 15–20 RM）
- **漸進超載原則（Progressive overload）**：逐步增加負荷
- **特異性原則（Specificity）**：訓練效果與訓練模式相關

### 12.3 DOMS（延遲性肌肉酸痛）

- 運動後 24–72 小時出現，常由**離心收縮**引起
- 非乳酸堆積，而是微小肌纖維損傷+發炎反應
- 自限性，輕度有氧活動可緩解

### 12.4 肌少症（Sarcopenia）

- 定義：年齡相關的骨骼肌量減少 + 肌力下降 ± 身體功能下降
- 診斷流程（亞洲共識 AWGS）：篩檢（握力、步速、小腿圍）→ 確認肌肉量（DXA / BIA）
    - 握力：男 <28 kg、女 <18 kg
    - 步速 <1.0 m/s
    - 小腿圍：男 <34 cm、女 <33 cm
...
- 急性期：**RICE → POLICE**
    ...
    - 現代觀念強調早期適量負重（optimal loading）優於完全休息

### 12.6 運動禁忌症

- 不穩定心絞痛、急性 MI 48 hr 內、未控制的心衰竭/心律不整/主動脈瓣狹窄、急性 PE/DVT、急性主動脈剝離、急性感染/發燒
