...
| 疾病 | 病原 | 疹子特徵 | 重點 |
|------|------|----------|------|
| 麻疹（Measles） | Paramyxovirus | **Koplik spots**（頰黏膜白點，pathognomonic）→ maculopapular rash 由臉→軀幹→四肢向下蔓延 | 3C：Cough、Coryza、Conjunctivitis → 出疹；嚴重併發症：肺炎（最常見死因）、SSPE（數年後）、腦炎 |
...
| 水痘（Varicella） | VZV | **各期並存**（vesicle, pustule, crust 同時存在）、向心性分布（軀幹多）、搔癢 | 併發症：皮膚細菌感染（GAS）、腦炎（小腦 ataxia）、肺炎（成人嚴重）；**Reye syndrome**（若用 aspirin）；免疫缺損 → disseminated |
| 手足口病 | Coxsackie A16、**Enterovirus 71** | 口腔潰瘍 + 手掌、腳掌、臀部 vesicular rash | EV71 可致嚴重：**腦幹腦炎、肺水腫（神經性）、心肌炎** → 台灣重要 |
...
| 猩紅熱（Scarlet fever） | **GAS** | 砂紙樣疹（sandpaper-like）、**Pastia lines**（皺褶處加深）、**草莓舌** | Pharyngitis 後 1–2 天出疹；治療：Penicillin/Amoxicillin |
...
- **兒童最常見需抗生素的感染**
- 好發年齡：6 個月–3 歲
- 最常見病原：**S. pneumoniae**（最常見）、nontypeable H. influenzae、M. catarrhalis
...
- 細菌性最重要：**GAS**
    - 好發 5–15 歲
    - 臨床：突發高燒、劇烈喉嚨痛、扁桃腺腫大/膿性滲出物、前頸淋巴結腫、無咳嗽/流鼻涕（區別病毒性）
...
- 治療：
    - 輕度：cool mist、觀察
    - 中度：**Dexamethasone（單劑 0.6 mg/kg）**（最有效治療）
    - 嚴重（明顯呼吸窘迫）：Racemic epinephrine（吸入）+ Dexamethasone
    - Epinephrine 效果短暫（1–2 hr），需觀察 rebound

#### 急性會厭炎（Epiglottitis）

- 好發年齡：2–7 歲
- 臨床：突發高燒、劇烈喉嚨痛、吞嚥困難（drooling）、**tripod position**（坐姿前傾、嘴巴張開）、muffled voice（不是 hoarse）、**無 barking cough**
- XR（lateral neck）：**Thumbprint sign**（會厭腫脹）
- 處理：
...
- 預防：
    - **Palivizumab（Synagis）**：anti-RSV monoclonal Ab，用於高危險嬰兒（每月 IM 注射，RSV 季節）
    - **Nirsevimab**：長效 anti-RSV mAb，單劑即可
    - 手部衛生、接觸隔離

#### 兒童社區型肺炎（Community-Acquired Pneumonia, CAP）

| 年齡層 | 最常見病原 | 特殊考量 |
|--------|----------|----------|
| 新生兒 | GBS、E. coli、Listeria | 經驗性：Ampicillin + Gentamicin |
| 1–3 個月 | Chlamydia trachomatis、RSV、S. pneumoniae | Chlamydia：staccato cough、無發燒、eosinophilia → Azithromycin |
| 3 個月–5 歲 | **Virus 最常見**；細菌以 **S. pneumoniae** 為主 | 典型：Amoxicillin；若 atypical → Azithromycin |
| ≥ 5 歲 | **Mycoplasma pneumoniae**、S. pneumoniae | Walking pneumonia、乾咳、headache → **Azithromycin / Doxycycline（≥ 8 歲）** |

- **Mycoplasma pneumoniae**：
    - 學齡兒童最常見非典型肺炎
    - 臨床：逐漸發病、乾咳、低度發燒、headache、malaise
...
- 細菌性：
    ...
    - Shigella：少量菌即可致病、seizure（小兒）、Shiga toxin → HUS 可能
    - **EHEC（O157:H7）**：不發燒或低燒 + 血性腹瀉 + **HUS**（溶血性尿毒症候群）
        - **禁用抗生素**（增加 HUS 風險！）
        - HUS triad：microangiopathic hemolytic anemia（schistocytes）+ thrombocytopenia + acute renal failure
...
- **輕-中度**：**口服補液（ORS）** → WHO ORS 配方、少量多次
- **重度或無法口服**：IV **N/S 或 LR 20 mL/kg bolus** → reassess → repeat if needed
- **嬰幼兒脫水最常見類型**：等張脫水（isonatremic）
...
- 診斷：
    - **尿液檢體收集**：2 歲以下 → catheterization 或 suprapubic aspiration（最準確）
    - UA：pyuria（WBC > 5/HPF）、nitrite (+)、leukocyte esterase (+)
    - **Urine culture（確診）**：catheter ≥ 50,000 CFU/mL；SPA ≥ any growth
- 影像學：
    - **首次發燒性 UTI（< 2 歲）**：**Renal ultrasound**（評估結構異常）
    - **VCUG（Voiding Cystourethrography）**：評估 **VUR（膀胱輸尿管逆流）** → 若 US 異常、反覆 UTI、atypical UTI 時施行
- **VUR（Vesicoureteral Reflux）**：
    - 兒童 UTI 最常見結構異常因素
    - Grade I–II：多數自行改善（隨年齡增長）
    - Grade III–V：可能需手術
    - 預防性抗生素使用仍有爭議
- 治療：
...
| 3 個月–成人 | **S. pneumoniae**（最常見）、N. meningitidis |
...
- 治療：
    ...
    - \> 1 個月：**Ceftriaxone + Vancomycin**（經驗性，covering resistant pneumococcus）
    - **Dexamethasone**：H. influenzae 腦膜炎建議使用（減少聽障）；S. pneumoniae 也可考慮
...
- 急性期：CRP↑、ESR↑、WBC↑（neutrophilia）、PLT↑（亞急性期 thrombocytosis，第 2–3 週）、ALT 輕度↑、pyuria（sterile pyuria）、hypoalbuminemia、anemia
...
- **Aspirin**：高劑量（抗發炎）→ 退燒後改低劑量（3–5 mg/kg/day，抗凝血）→ 持續 6–8 週（無冠脈異常）或更久（有異常）
...
| 疾病 | 遺傳 | 缺陷 | 特點 |
|------|------|------|------|
| **SCID** | X-linked（IL-2Rγ chain 缺陷最常見）或 AR（ADA deficiency 等） | T cell 嚴重缺乏 ± B cell/NK cell | 出生後數月內反覆嚴重感染（細菌、病毒、黴菌、PJP）；FTT；**活疫苗致命**（BCG、Rotavirus）；治療：**HSCT（造血幹細胞移植）**為唯一根治 |
| Wiskott-Aldrich syndrome | X-linked | **WAS protein** | **Triad：濕疹（eczema）+ 血小板減少（small platelets）+ 免疫缺損**；IgM↓、IgA↑、IgE↑ |
...
| Leukocyte adhesion deficiency（LAD） | AR | β₂ integrin（CD18）缺陷 → WBC 無法黏附/遷移 | 臍帶脫落延遲（> 30 天）、反覆感染但**無化膿**、WBC 極高（因無法進入組織） |
...
- 特徵：搔癢（itch is the hallmark）、乾燥、慢性反覆、lichenification
- 治療：
    - 保濕劑（最基本）
...
- **牛奶蛋白過敏（CMPA）**：
    ...
    - Non-IgE-mediated：嬰兒腸炎（blood-streaked stool）、FPIES
- 診斷：skin prick test、specific IgE、oral food challenge（gold standard）
...
- CXR：
    ...
    - 若 ball-valve（部分阻塞，較常見）：**患側 hyperinflation（air trapping）**、吐氣時更明顯（expiratory film 或 lateral decubitus 檢查）
    - 多數異物為 radiolucent（不顯影）
- 處理：
    - **Rigid bronchoscopy**（最佳診斷及治療）
    - 若完全阻塞致窒息：< 1 歲 → back blows + chest thrusts；≥ 1 歲 → Heimlich maneuver

### 6.3 囊腫性纖維化（Cystic Fibrosis, CF）
...
- 胎兒循環特殊結構：
    ...
    - **Ductus arteriosus（PDA）**：肺動脈 → 主動脈（bypassing lungs）
- 出生後轉變：
...
| **Coarctation of Aorta** | **Turner syndrome** 關聯；上肢血壓 > 下肢、股動脈脈搏弱/延遲；新生兒 duct-dependent 可致 shock；**differential cyanosis**（下肢發紺、上肢不發紺） | Systolic murmur, left interscapular area；rib notching（older children XR） |
...
- CXR：**Boot-shaped heart（coeur en sabot）**、肺血流減少（pulmonary oligemia）
- **Tet spell（hypercyanotic spell）**：突發嚴重發紺 + 哭鬧/運動/晨起時
    - 處理：**Knee-chest position**（增加 SVR → 減少 R→L shunt）、O₂、morphine（calming）、phenylephrine（增加 SVR）、IV fluid
    - 嬰兒常自行採取 **squatting position**（同理）
- 治療：complete repair（通常 3–6 個月大）；若無法早期修補 → BT shunt（姑息性）
...
- **PGE₁ 副作用**：apnea（最重要！需準備 intubation）、fever、hypotension、flushing、讓 TAPVR obstructed type 加劇（增加肺血流回流至已阻塞的肺靜脈系統）
...
- QTc > 470 ms（男）或 > 480 ms（女）
- 風險：Torsades de Pointes → VF → sudden cardiac death
- 先天性：Romano-Ward（AD，較常見）、Jervell-Lange-Nielsen（AR，伴感音性聽障）
- 後天性：藥物（抗心律不整、macrolides、抗精神病藥）、電解質（低 K、低 Mg、低 Ca）
- 治療：β-blocker（首選）、避免誘因藥物；嚴重者 ICD

### 7.6 心肌炎（Myocarditis）

- 最常見病因（兒童）：病毒性（**Coxsackievirus B** 最常見、Adenovirus）
...
- **嬰幼兒最常見腸阻塞原因**（6 個月–3 歲，peak 5–9 個月）
...
- 治療：
    ...
    - 禁忌：peritonitis、perforation、shock → 直接手術
...
- 診斷：
    ...
    - 確診：**小腸切片 → villous atrophy + crypt hyperplasia + intraepithelial lymphocytes**
    - 需在 gluten-containing diet 下檢測才準確
