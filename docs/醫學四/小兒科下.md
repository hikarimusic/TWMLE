...
- 最常見遺傳性腎病
- 基因：PKD1（chromosome 16，85%，較嚴重）、PKD2（chromosome 4）
- 臨床：
    - 雙側腎臟多發性囊腫，通常成人發病
    - 高血壓（常為最早表現）、腰痛、血尿、UTI、腎結石
- 腎外表現：肝臟囊腫（最常見腎外表現）、**腦動脈瘤（berry aneurysm → SAH）**、MVP、大腸憩室
- 診斷：腎臟超音波（年齡相關標準）
...
- 新生兒/嬰兒期發病，雙側腎臟顯著腫大
- 合併**先天性肝纖維化（congenital hepatic fibrosis）**
- 嚴重者：胎兒期少尿 → 羊水過少 → **Potter sequence**（肺發育不全、肢體壓迫變形、特殊面容）
- 治療：支持性
...
- 分類：
    - Primary（從未乾過）vs Secondary（曾乾 ≥ 6 個月後復發 → 需找潛在原因如 UTI、DM、壓力事件）
    - Monosymptomatic（僅夜尿）vs Non-monosymptomatic（合併日間症狀如頻尿、急尿、遺尿）
- 病因：多因素 — 夜間 ADH 分泌不足、膀胱功能性容量小、覺醒障礙、遺傳（父母一方有 → 44%；雙方 → 77%）
- 評估：病史 + 排尿日記 + UA（排除 UTI、DM、腎臟濃縮功能異常）；non-monosymptomatic 需進一步影像評估
- 治療：
    - 第一步：衛教與行為治療（限制睡前水分、規律排尿、正向鼓勵，**避免處罰**）
    - **Enuresis alarm（尿床警報器）**：長期成功率最高（first-line）
    - **Desmopressin（DDAVP）**：夜間 ADH 不足者效果佳；見效快但停藥復發率高；使用期間須限水避免低血鈉
...
- **中樞性（GnRH-dependent, true）**：最常見
    - 女生多為 idiopathic；男生需排除 CNS lesion（hypothalamic hamartoma 最常見病理性原因）
    - 檢查：LH、FSH↑；GnRH stimulation test → **LH 明顯上升**（LH-dominant response）
    - Brain MRI：排除 CNS pathology（尤其男生、< 6 歲女生）
    - 治療：**GnRH agonist**（如 leuprolide）→ 持續刺激導致 downregulation
- **週邊性（GnRH-independent, pseudo）**：性激素來源非下視丘-腦垂體軸
    ...
    - 原因：
        ...
        - 男：先天性腎上腺增生（CAH）、Leydig cell tumor、HCG-secreting tumor（hepatoblastoma）
    - 治療：針對病因
...
- **Appearance**（最重要）：口訣 **TICLS** — Tone、Interactiveness、Consolability、Look/gaze、Speech/cry
- **Work of Breathing**：nasal flaring、retractions、grunting、head bobbing、tripoding
- **Circulation to skin**：pallor、mottling、cyanosis
- 組合判讀：
    - WOB 異常 + 其餘正常 → 呼吸窘迫（代償中）
    - Appearance + WOB 異常 → 呼吸衰竭
    - Appearance + Circulation 異常 → 失代償性休克
    - 三項皆異常 → 心肺衰竭 / 即將 arrest
...
- **Primary assessment（ABCDE）**：Airway → Breathing（RR、SpO₂）→ Circulation（HR、BP、CRT < 2 秒）→ Disability（AVPU/GCS、瞳孔、血糖）→ Exposure（體溫、全身檢視）
    - 每一步發現 life-threatening problem → 立即介入再繼續
...
- 兒童心跳停止最常見原因：**呼吸衰竭**（成人多為心因性）→ 兒童 CPR 更重視 airway/breathing
...
| 壓通比（單人） | **30:2** | **30:2** | 30:2 |
| 壓通比（雙人） | **15:2** | **15:2** | 30:2 |
...
- 身體虐待紅旗：
    ...
    - 嬰兒 **subdural hematoma + retinal hemorrhage** → 高度懷疑 **搖晃嬰兒症候群（Abusive Head Trauma）**
